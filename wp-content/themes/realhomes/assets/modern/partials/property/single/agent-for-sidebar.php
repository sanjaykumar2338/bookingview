...
function display_sidebar_agent_box( $args ) {
	...
	if ( empty( $args['agent_id'] ) && empty( $args['author_id'] ) ) {
		return '';
	}
	...
	$agent_detail_page_link               = get_permalink( $agent_id );
    ...
    <section class="widget rh_property_agent<?php echo esc_attr( $agent_section_classes ); ?> <?php echo esc_attr( ( ! empty( $args['agent_class'] ) ) ? $args['agent_class'] : '' ); ?>">
		...
		if ( isset( $args['display_author'] ) && ( $args['display_author'] ) ) {
			...
			if ( has_post_thumbnail( $agent_id ) ) {
				?>
                <a class="agent-image" href="<?php echo esc_url( $agent_detail_page_link ); ?>">
		...
		if ( isset( $args['agent_title_text'] ) && ! empty( $args['agent_title_text'] ) ) {
            ...
            <h3 class="rh_property_agent__title">
                <a href="<?php echo esc_url( $agent_detail_page_link ); ?>">
                    <?php
		...
		if ( 'true' === $theme_display_agent_detail_page_link ) :
			...
			if ( isset( $args['display_author'] ) && ( $args['display_author'] ) ) : ?>
                <a class="rh-btn rh-btn-primary rh_property_agent__link" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>">
					<?php echo esc_html( $theme_agent_detail_page_link_text ); ?>
                </a>
			<?php
			else :
				?>
                <a class="rh-btn rh-btn-primary rh_property_agent__link" href="<?php echo esc_url( $agent_detail_page_link ); ?>">
					<?php echo esc_html( $theme_agent_detail_page_link_text ); ?>
                </a>
			<?php
			endif;
		...
		if ( 'true' === $inspiry_property_agent_form ) :
			if ( inspiry_get_agent_custom_form( $agent_id ) ) :
				inspiry_agent_custom_form( $agent_id );
            elseif ( isset( $args['agent_email'] ) && ! empty( $args['agent_email'] ) ) :
				$agent_form_id = 'agent-form-id' . $agent_id;
				?>
                    ...
                    <form id="<?php echo esc_attr( $agent_form_id ); ?>" class="rh_widget_form agent-form" method="post" action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>">
                        ...
                        <p class="rh_widget_form__row">
                            ...
                            <textarea cols="40" rows="6" type="text" name="message" placeholder="<?php esc_html_e( 'Message', 'framework' ); ?>" class="required" title="<?php esc_html_e( '* Please provide your message', 'framework' ); ?>"><?php
								printf( esc_textarea( realhomes_get_agent_default_message() ), esc_html( get_the_title( $property_id ) ) );
								?>
                            </textarea>
                        </p><!-- /.rh_widget_form__row -->
						<?php
