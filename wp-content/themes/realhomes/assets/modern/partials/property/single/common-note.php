...
if ( 'true' === $display_common_note ) {
	...
	if ( ! empty( $common_note_title ) || ! empty( $common_note ) ) {
		?>
		<div class="rh_property__common_note <?php realhomes_printable_section( 'common-note' ); ?>">
			<?php
			// Common note title.
			...
			if ( ! empty( $common_note ) ) {
                ?><div class="rh-property-note-inner-wrap"><p><?php echo esc_html( $common_note ); ?></p></div><?php
			}
			?>
		</div>
		<?php
