				...
				foreach ( $user_fields as $field ) {
					...
					if ( ! empty( $field['id'] ) && ! empty( $field['name'] ) ) {
						...
						if ( $field['type'] != 'checkbox' ) {
							$field_class = 'rh_form--1-column rh_form--columnAlign';
						} else {
							$field_class = 'rh_form_field_cb';
						}

						?>
                        <div class="rh_form__row additional_user_fields">
                            <div class="rh_form__item <?php echo esc_attr( $field_class ); ?>">
								...
								if ( ! empty( $field['type'] ) && $field['type'] == 'select' && ! empty( $field['options'] ) && is_array( $field['options'] ) ) {
									...
									?>
                                    <input class="rh_modern_input_cb" type="checkbox" id="<?php echo esc_attr( $field['id'] ); ?>" name="<?php echo esc_attr( $field['id'] ); ?>" value="<?php echo esc_attr( $field['std'] ); ?>">
                                    <label class="rh_modern_label_cb" for="<?php echo esc_attr( $field['id'] ); ?>"><span><?php echo esc_attr( $field['title'] ); ?></span></label>
									<?php
								} else {
									?>
