    ...
    <div class="agent-detail clearfix<?php echo esc_attr( $agent_section_classes ); ?>">
        <div class="left-box">
            ...
            if ( isset( $args['agent_title_text'] ) && !empty( $args['agent_title_text'] ) ) {
                ...
                <h3>
                    <?php
                    echo esc_html( $args['agent_title_text'] );
                    if ( isset( $args['agent_id'] ) && 0 < intval( $args['agent_id'] ) ) {
            ...
            <div class="agent-detail-inner-wrap clearfix">
                    ...
                    <ul class="contacts-list">
						...
						if ( isset( $args['agent_office_phone'] ) && ! empty( $args['agent_office_phone'] ) ) {
                            ...
                            <li class="office">
								...
								?> :
                                <a href="tel:<?php echo esc_attr( $args['agent_office_phone'] ); ?>"><?php echo esc_html( $args['agent_office_phone'] ); ?></a>
                            </li>
							<?php
						}
						if ( isset( $args['agent_mobile'] ) && ! empty( $args['agent_mobile'] ) ) {
                            ...
                            <li class="mobile">
								...
								?> :
                                <a href="tel:<?php echo esc_attr( $args['agent_mobile'] ); ?>"><?php echo esc_html( $args['agent_mobile'] ); ?></a>
                            </li>
							<?php
						}
						if ( isset( $args['agent_office_fax'] ) && ! empty( $args['agent_office_fax'] ) ) {
                            ...
                            <li class="fax">
								...
								?> :
                                <a href="fax:<?php echo esc_attr( $args['agent_office_fax'] ); ?>"><?php echo esc_html( $args['agent_office_fax'] ); ?></a>
                            </li>
							<?php
						}
						if ( isset( $args['agent_whatsapp'] ) && ! empty( $args['agent_whatsapp'] ) ) {
                            ...
                            <li class="whatsapp">
								...
								?> :
                                <a href="https://wa.me/<?php echo esc_attr( $args['agent_whatsapp'] ); ?>"><?php echo esc_html( $args['agent_whatsapp'] ); ?></a>
                            </li>
							<?php
						}
						?>
                ...
                <p>
					...
					<?php if ( 'true' === $theme_display_agent_detail_page_link ) {
						...
						if ( isset( $args['display_author'] ) && ( $args['display_author'] ) ) {
							?>
                            <a class="real-btn" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"><?php echo esc_html( $theme_agent_detail_page_link_text ); ?></a>
							<?php
						} else {
							?>
                            <a class="real-btn" href="<?php echo get_permalink( $args['agent_id'] ); ?>"><?php echo esc_html( $theme_agent_detail_page_link_text ); ?></a>
							<?php
						}
					}
					?>
            ...
            <div class="contact-form">
				...
				if ( inspiry_get_agent_custom_form( $args['agent_id'] ) ) {
                    ...
                    <form id="<?php echo esc_attr( $agent_form_id ); ?>" class="agent-form contact-form-small" method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>">
                        ...
                        <input type="text" name="email" placeholder="<?php esc_html_e( 'Email', 'framework' ); ?>" class="email required" title="<?php esc_html_e( '* Please provide valid email address', 'framework' ); ?>">
                        <input type="text" name="phone" placeholder="<?php esc_html_e( 'Phone', 'framework' ); ?>" class="digits required" title="<?php esc_html_e( '* Please provide valid phone number', 'framework' ); ?>">
                        <textarea name="message" class="required" placeholder="<?php esc_html_e( 'Message', 'framework' ); ?>" title="<?php esc_html_e( '* Please provide your message', 'framework' ); ?>"><?php
                        ...
                        <div class="agent-form-contact-methods-wrapper">
							...
							<?php
							$agent_contact_links = get_option( 'realhomes_agent_contact_links', 'true' );
                            if ( 'true' === $agent_contact_links && ( ( isset( $args['agent_mobile'] ) && ! empty( $args['agent_mobile'] ) ) || ( isset( $args['agent_whatsapp'] ) && ! empty( $args['agent_whatsapp'] ) ) ) ) {
	                            realhomes_property_agent_contact_methods( $args['agent_mobile'], $args['agent_whatsapp'], 'submit-button real-btn' );
                            } ?>
	...
	if ( $agent_display_option == "my_profile_info" ) {
		...
		if ( ! empty( $property_agents ) ) {
			...
			foreach ( $property_agents as $agent ) {
				if ( 0 < intval( $agent ) ) {
					$agent_args                       = array();
					$current_agent                    = intval( $agent );
					$agent_args['agent_id']           = apply_filters( 'wpml_object_id',  $current_agent, 'agent', false );
					$agent_args['agents_count']       = $agents_count;
					$agent_args['agent_title_text']   = esc_html( get_option( 'inspiry_property_agent_label', esc_html__( 'Agent', 'framework' ) ) ) . " " . esc_html( get_the_title( $agent_args['agent_id'] ) );
