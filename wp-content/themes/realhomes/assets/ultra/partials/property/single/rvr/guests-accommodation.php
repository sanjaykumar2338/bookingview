                ...
                <ul>
					...
					foreach ( $beds_details as $bed_detail ) {
						if ( ! empty( $bed_detail['room_type'] ) && ! empty( $bed_detail['bed_type'] ) && ! empty( $bed_detail['beds_number'] ) && ! empty( $bed_detail['guests_number'] ) ) {
							?>
                            <li><i class="fas fa-bed"></i><strong><?php echo esc_html( $bed_detail['room_type'] ); ?>:</strong> <?php echo intval( $bed_detail['beds_number'] ) . ' ' . esc_html( $bed_detail['bed_type'] ) . ' <i class="guests-info">(' . intval( $bed_detail['guests_number'] ) . ' ' . esc_html__( 'guests', 'framework' ) . ')</i>'; ?></li>
                            <?php
						}
					}
					?>
