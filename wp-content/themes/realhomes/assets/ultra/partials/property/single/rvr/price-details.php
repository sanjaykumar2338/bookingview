...
if ( inspiry_is_rvr_enabled() && 'true' === get_option( 'inspiry_price_details_display', 'true' ) ) {
	$property_id       = get_the_ID();
	$bulk_prices       = get_post_meta( $property_id, 'rvr_bulk_pricing', true );
	$service_charges   = get_post_meta( $property_id, 'rvr_service_charges', true );
	$govt_tax          = get_post_meta( $property_id, 'rvr_govt_tax', true );
	...
	if ( ! empty( $bulk_prices ) || ! empty( $service_charges ) || ! empty( $govt_tax ) || ! empty( $additional_fees ) || ! empty( $extra_guest_price ) ) {
