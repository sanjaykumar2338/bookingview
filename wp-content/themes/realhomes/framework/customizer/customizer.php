...
if ( ! function_exists( 'inspiry_load_customize_controls' ) ) :
	...
	function inspiry_load_customize_controls() {
		...
		require_once( INSPIRY_FRAMEWORK . 'customizer/custom/control-notice.php' );
	}
...
if ( ! function_exists( 'realhomes_get_elementor_library' ) ) {
	/**
	 * Get List of Elementor Published Templates
	 *
	 * @since RealHomes 3.18.0
	 *
	 * @return array
	 */
	function realhomes_get_elementor_library() {

		$return_array = array( 'default' => esc_html__( 'Default', 'framework' ) );
		if ( 'ultra' === INSPIRY_DESIGN_VARIATION ) {
			$return_array = array( 'default' => esc_html__( 'None', 'framework' ) );
		}
...
if ( 'classic' !== INSPIRY_DESIGN_VARIATION ) {
	/**
	 * Property Meta Custom Icons Settings
	 */
	require_once( INSPIRY_FRAMEWORK . 'customizer/property-meta-icons.php' );
}
 ...
 * Misc Settings
 */
require_once( INSPIRY_FRAMEWORK . 'customizer/advertisement-slots.php' );
