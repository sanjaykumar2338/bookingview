	...
	class ERE_Purchase_API {
		...
		public function verify_purchase_code( $code ) {
			...
			if ( empty( $code ) ) {
				...
				$error->add( 'error', esc_html__( 'Wrong format! Please enter an item purchase code with correct format.', 'framework' ) );
			...
			if ( ! is_wp_error( $envato_purchases ) && is_string( $envato_purchases['body'] ) ) {
				...
				if ( isset( $purchases_body['item'] ) ) {
					...
					if ( isset( $item_info['id'] ) && '5373914' === str_replace( ' ', '', $item_info['id'] ) ) {
						...
						$error->add( 'error', esc_html__( 'Something went wrong :(', 'framework' ) );
				...
				$error->add( 'error', esc_html__( 'Problem in connecting... Please try again in a few minutes!', 'framework' ) );
