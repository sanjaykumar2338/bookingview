...
if ( ! function_exists( 'realhomes_assign_image_to_property' ) ) {
	/**
	 * This function will create an association between the uploaded image and the property
	 *
	 * @since 3.20.0
	 * 
	 * @param int $property_id required property id
	 * @param int $image_id required image id
	 *
	 * @return bool
	 */
	function realhomes_assign_image_to_property( $property_id, $image_id ) {
		if ( ! empty( $property_id ) && ! empty( $image_id ) ) {
			global $wpdb;
			$wp_posts = $wpdb->prefix . "posts";
			$wpdb->query( $wpdb->prepare( "UPDATE $wp_posts SET post_parent = %d WHERE ID = %s", $property_id, $image_id ) );
			return true;
		}
	}
}
