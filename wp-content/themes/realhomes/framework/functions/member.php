...
if ( ! function_exists( 'inspiry_get_gravatar' ) ) {
	...
	function inspiry_get_gravatar( $email, $size ) {
		...
		// Get Default Avatar from Global Settings
		$default_avatar = get_option( 'avatar_default' );

		// Check if it is set to Default Avatar or Blank Avatar
		$fallback_avatar = ( 'gravatar_default' === $default_avatar || 'blank' === $default_avatar ) ? '&r=g' : '&d=' . $default_avatar;

		// Convert email into md5 hash and set image size to 200 px.
		$gravatar_url = 'https://www.gravatar.com/avatar/' . md5( $email ) . '?s=' . $size . $fallback_avatar;
...
if ( ! function_exists( 'realhomes_add_user_roles' ) ) {
	/**
	 * Add additional user roles allowed in settings
	 *
	 * @since 4.3.0
	 */
	function realhomes_add_user_roles() {
		$user_roles = get_option( 'ere_allowed_user_roles', array( 'agent' => true, 'agency' => true ) );
		if ( is_array( $user_roles ) && 0 < count( $user_roles ) ) {
			foreach ( $user_roles as $role => $state ) {
				...
				add_role( $role, $role_string, array( 'read' => true, 'level_0' => true ) );
			}
		}
	}

	add_action( 'init', 'realhomes_add_user_roles' );
}
...
if ( ! function_exists( 'inspiry_user_sync_roles' ) ) {
	...
	function inspiry_user_sync_roles() {
		...
		$user_roles      = get_option( 'ere_allowed_user_roles', [ 'agent' => true, 'agency' => true ] );
		if ( is_array( $user_roles ) && count( $user_roles ) ) {
			foreach ( $user_roles as $role => $state ) {
				...
				$user_sync_roles[ $role ] = $role_string;
			}
		}
...
if ( ! function_exists( 'realhomes_add_user_approval_status_field' ) ) {
	/**
	 * Adds user approval status field to users page
	 *
	 * @since 4.3.0
	 *
	 * @param $user
	 */
	function realhomes_add_user_approval_status_field( $user ) {
		?>
        <table class="form-table">
            <tr>
                <th>
                    <label for="realhomes-user-approval-field"><?php esc_html_e( 'User Approval Status', 'framework' ); ?></label>
                </th>
                <td>
					<?php
                    if ( $user === 'add-new-user' || $user === 'add-existing-user' ) {
	                    $current_user_status = $is_current_user = 0;
                    } else {
	                    $current_user_status = get_user_meta( $user->ID, 'ere_user_approval_status', true );
	                    $is_current_user     = $user->ID === get_current_user_id();
                    }

					$all_statuses        = array(
						0 => esc_html__( 'None', 'framework' ),
						'approved' => esc_html__( 'Approved', 'framework' ),
						'denied'   => esc_html__( 'Denied', 'framework' ),
						'pending'  => esc_html__( 'Pending', 'framework' )
					);
					?>
                    <select name="realhomes_user_approval_field" id="realhomes-user-approval-field" <?php echo $is_current_user ? 'disabled' : ''; ?>>
						<?php
						foreach ( $all_statuses as $key => $status ) {
							?>
                            <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $current_user_status, $key, true ); ?>><?php echo esc_html( $status ); ?></option>
							<?php
						}
						?>
                    </select>
					...
					if ( $is_current_user ) {
						?>
                        <p class="desc"><?php esc_html_e( 'You can not change the status for yourself.', 'framework' ); ?></p>
						<?php
					}
					?>
                </td>
	...
	add_action( 'show_user_profile', 'realhomes_add_user_approval_status_field', 5 );
	add_action( 'edit_user_profile', 'realhomes_add_user_approval_status_field', 5 );
	add_action( 'user_new_form', 'realhomes_add_user_approval_status_field', 5 );
}

if ( ! function_exists( 'realhomes_update_user_approval_status' ) ) {
	/**
	 * Update user approval status field
	 *
	 * @since  4.3.0
	 *
	 * @param $user_id
	 ...
	 * @return bool
	 */
	function realhomes_update_user_approval_status( $user_id ) {

		if ( ! current_user_can( 'edit_user', $user_id ) ) {
			return false;
		}

		if ( ! empty( $_POST['realhomes_user_approval_field'] ) ) {
			update_user_meta( $user_id, 'ere_user_approval_status', sanitize_text_field( $_POST['realhomes_user_approval_field'] ) );
		}

	}

	add_action( 'personal_options_update', 'realhomes_update_user_approval_status' );
	add_action( 'edit_user_profile_update', 'realhomes_update_user_approval_status' );
	add_action( 'user_register', 'realhomes_update_user_approval_status' );
}