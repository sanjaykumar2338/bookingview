	...
	function theme_pagination( $pages = '' ) {
		...
		if ( 1 != $pages ) {
			...
			if ( $paged > 1 && $show_items < $pages ) {
				echo "<a rel='prev' href='" . get_pagenum_link( $prev ) . "' class='real-btn real-btn-jump rh_arrows_left' > " . esc_html__( 'Previous', 'framework' ) . "</a> ";
			}
			...
			if ( $paged < $pages && $show_items < $pages ) {
				echo "<a rel='next' href='" . get_pagenum_link( $next ) . "' class='real-btn real-btn-jump rh_arrows_right' >" . esc_html__( 'Next', 'framework' ) . "</a> ";
			}
...
if ( ! function_exists( 'inspiry_theme_pagination' ) ) {
	...
	function inspiry_theme_pagination( $pages = '' ) {
		...
		if ( 1 != $pages ) {
			...
			if ( $paged > 1 && $show_items < $pages ) {
				echo "<a rel='prev' href='" . get_pagenum_link( $prev ) . "' class='rh_pagination__btn rh_pagination__prev' >";
			...
			if ( $paged < $pages && $show_items < $pages ) {
				echo "<a rel='next' href='" . get_pagenum_link( $next ) . "' class='rh_pagination__btn rh_pagination__next' >";
...
if ( ! function_exists( 'realhomes_ajax_pagination' ) ) {
	 ...
	 * @modified 4.1.0
	 */
	function realhomes_ajax_pagination( $pages = '', $current_query = '', $wrap_class = '' ) {

		echo '<div class="svg-loader">';
		echo '<img src="' . INSPIRY_DIR_URI . '/images/loading-bars.svg" width="32" height="32" alt="' . esc_html( 'Loading...', 'framework' ) . '" />';
		echo '</div>';
		...
		if ( 1 != $pages ) {
			...
			if ( ( $paged > 2 ) && ( $paged > $range + 1 ) && ( $pages_to_show < $pages ) ) {
				echo "<a href='" . get_pagenum_link( 1 ) . "' class='rh-pagination-first-item " . $page_class . " real-btn-jump' data-page-number='" . 1 . "' data-page-nav='first'>&laquo; " . esc_html__( 'First', 'framework' ) . "</a> "; // First Page
			}

			if ( ( $paged > 1 ) && ( $pages_to_show < $pages ) ) {
				echo "<a rel='prev' href='" . get_pagenum_link( $prev ) . "' class='rh-pagination-prev-item " . $page_class . " real-btn-jump' data-page-number='" . intval( $prev ) . "' data-page-nav='previous'>&laquo; " . esc_html__( 'Previous', 'framework' ) . "</a> "; // Previous Page
			}
			...
			for ( $i = 1; $i <= $pages; $i ++ ) {
				if ( ( ( $i > $min_page_number ) && ( $i < $max_page_number ) ) || ( $pages <= $pages_to_show ) ) {
					...
					echo "<a href='" . get_pagenum_link( $i ) . "' class='" . $current_class . " nav-number' data-page-number='" . $i . "'>" . $i . "</a> ";
				}
			}

			if ( ( $paged < $pages ) && ( $pages_to_show < $pages ) ) {
				echo "<a rel='next' href='" . get_pagenum_link( $next ) . "' class='rh-pagination-next-item " . $page_class . " real-btn-jump' data-page-number='" . intval( $next ) . "' data-page-nav='next'>" . esc_html__( 'Next', 'framework' ) . " &raquo;</a> "; // Next Page
			}

			if ( ( $paged < $pages - 1 ) && ( $paged + $range - 1 < $pages ) && ( $pages_to_show < $pages ) ) {
				echo "<a href='" . get_pagenum_link( $pages ) . "' class='rh-pagination-last-item " . $page_class . " real-btn-jump' data-page-number='" . intval( $pages ) . "' data-page-nav='last'>" . esc_html__( 'Last', 'framework' ) . " &raquo;</a> "; // Last Page
			}
	...
	function inspiry_users_pagination( $pages ) {
		...
		if ( 1 != $pages ) {
			...
			if ( $paged > 1 && $showitems < $pages ) {
				echo "<a rel='prev' href='" . get_pagenum_link( $prev ) . "' class='real-btn' >&laquo; " . esc_html__( 'Previous', 'framework' ) . "</a> ";
			}
			...
			if ( $paged < $pages && $showitems < $pages ) {
				echo "<a rel='next' href='" . get_pagenum_link( $next ) . "' class='real-btn' >" . esc_html__( 'Next', 'framework' ) . " &raquo;</a> ";
			}
