<header class="rh-header-slim">
    <div class="rh-header-slim-left">
		<?php
		// Adding dashboard logo
		get_template_part( 'common/dashboard/partials/dashboard-logo' );
		?>
        <div id="rh-sidebar-menu-toggle" class="rh-sidebar-menu-toggle rh-menu-toggle" title="<?php esc_html_e( 'Open Sidebar Menu', 'framework' ); ?>">
    ...
    <div class="rh-header-slim-right">
        ...
        <ul class="rh-user-account">
            ...
            <li class="rh-user-account-profile-image">
				...
				if ( is_user_logged_in() ) {
					if ( isset( $current_user_meta['profile_image_id'] ) && ! empty( $current_user_meta['profile_image_id'][0] ) ) {
						...
						$gravtar = get_avatar( $current_user->user_email, '150', $global_avatar, $current_user->display_name, array( 'class' => 'user-icon' ) );
						if ( ! empty( $gravtar ) ) {
			...
			<?php
			if ( realhomes_get_current_user_role_option( 'property_submit' ) ) {
			...
			if ( 'true' === get_option( 'realhomes_dashboard_frontpage_button', 'false' ) ) {
				$button_label = get_option( 'realhomes_dashboard_frontpage_button_label', esc_html__( 'Front Page', 'framework' ) );
				if ( ! empty( $button_label ) )
					?>
                    <li class="rh-user-account-back-to-home-btn"><a class="btn btn-secondary" href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_html__( 'Home URL', 'framework' ); ?>"><?php echo esc_html( $button_label ); ?></a></li>
				<?php
			}
			?>
        </ul><!-- .rh-user-account -->
