...
class RHEAGoogleMapWidgetClass extends elementorModules.frontend.handlers.Base {
    ...
    bindEvents() {
        ...
        if ( typeof googlePropertiesData !== "undefined" ) {
            mapProperties = googlePropertiesData;
        }

        const mapSettings = this.getElementSettings();
        if ( 'yes' === mapSettings.map_sync_properties && typeof propertiesMapNewData !== "undefined" ) {
            mapProperties.rheaPropertiesData = propertiesMapNewData.rheaPropertiesData;
        }

        this.loadGoogleMap( mapProperties );

        jQuery( window ).on( 'rheaUpdateMapData', ( event ) => {
            if ( event.mapProperties ) {
                mapProperties.rheaPropertiesData = event.mapProperties;
                this.loadGoogleMap( mapProperties );
            }
        } );
    }

    loadGoogleMap( mapProperties ) {
        let mapWrapper           = this.elements.$mapDataDiv,
            wrapperID            = mapWrapper.attr( 'id' ),
            propertiesMapData    = JSON.parse( mapProperties.rheaPropertiesData ),
            propertiesMapOptions = JSON.parse( mapProperties.rheaMapOptions ),
            mapStuff             = JSON.parse( mapProperties.rheaMapStuff );
            ...
            if ( 0 < propertiesMapData.length ) {
                ...
                for ( let i = 0; i < propertiesMapData.length; i++ ) {
                    ...
                    if ( propertiesMapData[i].lat && propertiesMapData[i].lng ) {
                        ...
                        let infoWindowMaxWidth = 0;
                        let closeButtonMargin  = "0 0 -16px -16px";
                        let pixelOffsetX       = -122;
                        let pixelOffsety       = -48;
                        ...
                        if ( document.body.classList.contains( "design_ultra" ) ) {
                            ...
                            closeButtonMargin  = "8px 8px -24px -16px";
                            pixelOffsetX       = -225;
                            pixelOffsety       = -70;
                        ...
                        let infoWindowOptions = {
                            ...
                            zIndex                 : null,
                            closeBoxMargin         : closeButtonMargin,
