...
?>
<div class="rh-ultra-media-count">
	<?php if ( $get_prop_images > 0 ) { ?>
        <div class="rh-media rh-media-image"
             data-fancybox-trigger="gallery-<?php echo esc_attr( get_the_ID() ). '-' . esc_attr($this_widget_id); ?>"
	...
	if ( $count_videos > 0 ) {
		?>
        <div class="rh-media rh-media-video"
             data-fancybox-trigger="video-<?php echo esc_attr( get_the_ID() ). '-' . esc_attr($this_widget_id); ?>"
             data-this-id="<?php echo esc_attr( get_the_ID() ); ?>">
		...
		foreach ( $get_post_meta_image as $item ) {
			...
			if ( ! empty( $images_src[0] ) ) {
                ...
                <span style="display: none;"
                      data-fancybox="gallery-<?php echo esc_attr( get_the_ID() ). '-' . esc_attr($this_widget_id); ?>"
	...
	if ( ! empty( $inspiry_video_group ) ) {
		foreach ( $inspiry_video_group as $video ) {
			if ( ! empty( $video['inspiry_video_group_url'] ) ) {
				?>
                <span style="display: none;"
                        data-fancybox="video-<?php echo esc_attr( get_the_ID() ). '-' . esc_attr($this_widget_id); ?>"
