<?php
/**
 * This file is responsible for the purchase API.
 * 
...
if ( ! class_exists( 'ERE_Purchase_API' ) ) {
	...
	class ERE_Purchase_API {
		...
		public function verify_purchase_code( $code ) {
			...
			if ( empty( $code ) ) {
				$error->add( 'error', esc_html__( 'Please enter an item purchase code.', 'easy-real-estate' ) );

				return $error;
			} else if ( ! preg_match( '/^([a-f0-9]{8})-(([a-f0-9]{4})-){3}([a-f0-9]{12})$/i', $code ) ) {
				$error->add( 'error', esc_html__( 'Wrong format! Please enter an item purchase code with correct format.', 'easy-real-estate' ) );
			...
			if ( ! is_wp_error( $envato_purchases ) && is_string( $envato_purchases['body'] ) ) {
				...
				if ( isset( $purchases_body['item'] ) ) {
					...
					if ( isset( $item_info['id'] ) && '5373914' === str_replace( ' ', '', $item_info['id'] ) ) {
						...
						$error->add( 'error', esc_html__( 'Something went wrong :(', 'easy-real-estate' ) );
				...
				$error->add( 'error', esc_html__( 'Problem in connecting... Please try again in a few minutes!', 'easy-real-estate' ) );
