...
$inspiry_property_analytics_column      = $this->get_option( 'inspiry_property_analytics_column', 'disabled' );
...
if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'inspiry_ere_settings' ) ) {
	...
	update_option( 'inspiry_property_analytics_column', $inspiry_property_analytics_column );
}
?>
	...
	<form method="post" action="" novalidate="novalidate">
		<table class="form-table">
			<tbody>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Property Views Column', 'easy-real-estate' ); ?></th>
					<td>
						<?php
							$property_analytics_column = get_option( 'inspiry_property_analytics_column', 14 );
						?>
						<fieldset>
							<label>
								<input type="radio" name="inspiry_property_analytics_column" value="enabled" <?php checked( $property_analytics_column, 'enabled' ); ?>>
								<span><?php esc_html_e( 'Enable', 'easy-real-estate' ); ?></span>
							</label>
							<br>
							<label>
								<input type="radio" name="inspiry_property_analytics_column" value="disabled" <?php checked( $property_analytics_column, 'disabled' ); ?>>
								<span><?php esc_html_e( 'Disable', 'easy-real-estate' ); ?></span>
							</label>
						</fieldset>
					</td>
