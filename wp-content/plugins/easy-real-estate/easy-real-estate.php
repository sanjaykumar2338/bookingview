...
if ( ! class_exists( 'Easy_Real_Estate' ) ) :
	...
	final class Easy_Real_Estate {
		...
		public function enqueue_scripts() {
			...
			if (
                    (
                            inspiry_is_property_analytics_enabled()
                    ...
                    || (
                            ...
                            && 'show' === get_option( 'realhomes_dashboard_analytics_module', 'show' )
                    )
            ) {
            ...
            if (
	            ...
	            // Chart.js JS library for agent/agency stats doughnut chart
	            wp_enqueue_script( 'chart.js', ERE_PLUGIN_URL . 'includes/property-analytics/js/chart.min.js', array( 'jquery' ), $this->version, true );
            }
		}
		...
		public function tabs() {
			...
			$tabs = array(
				...
				'users'              => esc_html__( 'Users', 'easy-real-estate' ),
		...
		public function plugin_action_links( $links ) {
			$settings_link      = sprintf( '<a href="%1$s">%2$s</a>', admin_url( 'admin.php?page=ere-settings' ), esc_html__( 'Settings', 'easy-real-estate' ) );
			$documentation_link = sprintf( '<a href="%1$s" target="_blank">%2$s</a>', 'https://realhomes.io/documentation/price-format-settings/', esc_html__( 'Documentation', 'easy-real-estate' ) );
